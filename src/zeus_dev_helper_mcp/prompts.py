...
- **demo_beer_sample = beer-sample catalog UI.** Search follows travel: `rt.agent.run_turn` with `chat_request` omitted so `catalog.load_for_turn` merges SCOPE BRIEF + MINI-SCHEMA. An LLM key is required even when has_llm_key=false. Put LLM_API_KEY, XAI_API_KEY, or OPENAI_API_KEY in this process (checklist 1.2) and in the app .env (checklist 3.2). config.json llm.api_key_env stays that name. has_llm_key=true does not count. The BFF does not build a pipeline body. Do not clone `demo_travel_sample` for this bucket. Never the default unless the user named beer-sample or a beer catalog website.
...
5. validate_env / readiness_check / recommend_surface as next_step directs. validate_env reads LLM_API_KEY, XAI_API_KEY, or OPENAI_API_KEY from this process. set_prereq(has_llm_key=true) does not satisfy checklist 1.2. readiness_check does not mark 1.2 done without that variable when the app calls run_turn (travel, beer, API, yelp demo). Beer search needs the key even when has_llm_key=false.
...
9. Before smoke_test_agent or starting the sample, run verify_local_setup on the project directory. .env must contain a non-empty LLM_API_KEY (or the name in config.json). llm.api_key_env stays that variable name. Do not paste the secret into api_key_env. Then smoke_test_zeus, then smoke_test_agent when search is rt.agent.run_turn (travel and the beer catalog UI). Beer still must not clone demo_travel_sample. If has_llm_key=false, say an LLM key is required for that search rather than switching the beer app back to bare Direct verbs.
