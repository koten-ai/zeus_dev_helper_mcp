...
Coach (0.6): `recommend_surface` / `explain_verb` / `lint_verb_args` / `suggest_verb_call` / `diagnose_error` / `compat_check` / `bind_contract` / `lint_runtime_config`. Does **not** rewrite V1 `scaffold_app` / `smoke_test_agent`.
